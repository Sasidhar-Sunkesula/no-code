...
function FolderComponent({
  ...
  return (
    <div>
      ...
      >
        <ChevronRight
          className={`h-4 transform ${isOpen ? "rotate-90" : ""}`}
        />
        {name}
      </div>
...
export function FileExplorer({
  ...
  const [folders, setFolders] = useState<Folders[]>(
    buildHierarchy(templateFiles)
  );
  const [selectedFileName, setSelectedFileName] = useState<string>("");

  const [activeTab, setActiveTab] = useState<"code" | "output">("code");

  const toggleTab = useCallback((tab: "code" | "output") => {
    setActiveTab(tab);
  }, []);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      for (const file of filesFromState) {
        const existingFileIndex = templateFiles.findIndex(
          (f) => f.path === file.path
        );
  ...
  const handleFileClick = (name: string) => {
    ...
    console.log("files areee", folders);
  };
    ...
    <div className="flex h-[75vh]">
      ...
      <div className="flex flex-col">
        <div className="flex gap-5 bg-black">
          <p
            className="p-2 bg-white cursor-pointer text-black"
